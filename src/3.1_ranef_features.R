...
product_attrs <- c( "manufacturer", "group", "content", "unit", "pharmForm",
                    "genericProduct", "salesIndex", "category", "campaignIndex","rrp")
orig_train_df[, deduplicated_pid := pid[[1]], by = product_attrs]
#train_df <- orig_train_df[day<=63] #for model training
train_df <- orig_train_df[day<=92]
set.seed(1234)
...
string_vars <- c("deduplicated_pid", "lineID", "manufacturer", "group", "content", "unit", "pharmForm", 
                 "genericProduct", "salesIndex", "category", "campaignIndex")
train_df[, lapply(.SD, function(x) length(unique(x))),.SDcols = string_vars]

# encode pid
encode_pid_mxef <- glmmTMB(num_items_bought ~ (1|deduplicated_pid),
                           data= train_df, ziformula = ~1,
                           family= poisson)
pid_ref <- ranef(encode_pid_mxef)$cond$deduplicated_pid
setDT(pid_ref, keep.rownames = TRUE)
names(pid_ref) <- c("deduplicated_pid", "pid_ref")
saveRDS(pid_ref, "../../data/interim/train92d_ref/train92d_pid_ref")
rm(encode_pid_mxef)
# # 
# # # encode pid and day_mod_7 interaction effect
encode_pid_day7_mxef <- glmmTMB(num_items_bought ~ (1|day_mod_7/deduplicated_pid),
                                data= train_df, ziformula = ~1,
                                family= poisson)
pid_day7_ref <- ranef(encode_pid_day7_mxef)$cond$deduplicated_pid
setDT(pid_day7_ref, keep.rownames = TRUE)
names(pid_day7_ref) <- c("pid_day_mod_7", "pid_day7_ref")
pid_day7_ref[, c("deduplicated_pid", "day_mod_7") := tstrsplit(pid_day_mod_7, ":", fixed=TRUE)]
pid_day7_ref[, pid_day_mod_7:=NULL]
saveRDS(pid_day7_ref, "../../data/interim/train92d_ref/train92d_pid_day7_inter_ref")
day7_ref <- ranef(encode_pid_day7_mxef)$cond$day_mod_7
setDT(day7_ref, keep.rownames = TRUE)
names(day7_ref) <- c("day_mod_7", "day7_ref")
saveRDS(day7_ref, "../../data/interim/train92d_ref/train92d_pid_day7_ref")
rm(encode_pid_day7_mxef)
# # 
# # # encode pid and day_mod_10 interaction effect
encode_pid_day10_mxef <- glmmTMB(num_items_bought ~ (1|day_mod_10/deduplicated_pid),
                                 data= train_df, ziformula = ~1,
                                 family= poisson)
pid_day10_ref <- ranef(encode_pid_day10_mxef)$cond$deduplicated_pid
setDT(pid_day10_ref, keep.rownames = TRUE)
names(pid_day10_ref) <- c("pid_day_mod_10", "pid_day10_ref")
pid_day10_ref[, c("deduplicated_pid", "day_mod_10") := tstrsplit(pid_day_mod_10, ":", fixed=TRUE)]
pid_day10_ref[, pid_day_mod_10:=NULL]
saveRDS(pid_day10_ref, "../../data/interim/train92d_ref/train92d_pid_day10_inter_ref")
...
saveRDS(day10_ref, "../../data/interim/train92d_ref/train92d_pid_day10_ref")
rm(encode_pid_day10_mxef)

# ------------------------ random effect encoding for clustering ---------------------#
...
# saveRDS(manufacturer_ref, "../../data/interim/train63d_ref/train63d_manufacturer_ref")
...
# saveRDS(group_ref, "../../data/interim/train63d_ref/train63d_group_ref")
...
# saveRDS(content_ref, "../../data/interim/train63d_ref/train63d_content_ref")
...
# saveRDS(pharmForm_ref, "../../data/interim/train63d_ref/train63d_pharmForm_ref")
...
# saveRDS(category_ref, "../../data/interim/train63d_ref/train63d_category_ref")
...
# saveRDS(unit_ref, "../../data/interim/train63d_ref/train63d_unit_ref")
# rm(encode_unit_mxef)
...
# saveRDS(group_day10_ref, "data/interim/group_day10_inter_ref")
