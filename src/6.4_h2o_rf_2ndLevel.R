...
rf_params <- list( max_depth = seq(4, 10, 1),
                   
                   sample_rate = c(0.2,0.4,0.6,0.8),
                   
                   #min_rows = c(2,4,6),
                   
                   col_sample_rate_change_per_level = seq(0.5, 2.0, 0.5),
...
for (i in 1:4) {
  ...
  retrained_rf_rmse <- round(h2o.rmse(h2o.performance(retrained_rf, newdata = test_set.hex)),4)
  ...
  write_feather(preds_test77d, paste0("../data/preds2ndLevel/end77d_test_rf_",retrained_rf_rmse,'.feather'))
...
for (i in 1:4) {
  ...
  write_feather(preds_test92d, paste0("../data/preds2ndLevel/end92_test_rf_",retrained_rf_rmse,'.feather'))
